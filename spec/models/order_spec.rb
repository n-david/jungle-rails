...
RSpec.describe Order, type: :model do
  ...
  describe 'After creation' do
    ...
    it 'deducts quantity from products based on their line item quantities' do
      ...
      prev_quantity1 = @product1.quantity
      prev_quantity2 = @product2.quantity
      @order.save!
      # 4. reload products to have their updated quantities
      # @product1.reload
      # @product2.reload
      # 5. use RSpec expect syntax to assert their new quantity values
      expect(@product1.quantity).to eql(prev_quantity1 - LineItem.find_by(product: @product1).quantity)
      expect(@product2.quantity).to eql(prev_quantity2 - LineItem.find_by(product: @product2).quantity)
    end
    # pending test 2
